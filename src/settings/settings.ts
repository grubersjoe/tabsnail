...
const shrinkPageInput = document.getElementById('shrink-page') as HTMLInputElement
...
chrome.storage.sync.get<Partial<Settings>>(null, settings => {
  ...
  if (settings.shrinkPage) {
    shrinkPageInput.checked = settings.shrinkPage
  }
...
shrinkPageInput.addEventListener('input', () => {
  debounce(() => chrome.storage.sync.set<Settings>({ shrinkPage: shrinkPageInput.checked }), 100)()
})
